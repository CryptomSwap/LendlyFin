...
export type DisputeOpenedPayload = {
  ...
  listingTitle: string;
  bookingUrl: string;
  /** "renter" | "owner" | "admin" for recipient-specific copy */
...
/**
 * Hebrew RTL dispute-opened email HTML.
 */
export function renderDisputeOpenedEmail(payload: DisputeOpenedPayload): string {
  const { bookingRef, listingTitle, bookingUrl, recipientRole } = payload;

  const intro =
    recipientRole === "admin"
      ? "נפתחה מחלוקת בהזמנה. נדרש טיפול מנהל."
      : "נפתחה מחלוקת בהזמנה. הצוות יבדוק ויחזור בהקדם.";
...
export function getDisputeOpenedSubject(bookingRef: string): string {
  return `נפתחה מחלוקת בהזמנה – ${bookingRef}`;
}
