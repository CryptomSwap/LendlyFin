...
export interface RedesignInputProps
...
export function RedesignInput({
  ...
  return (
    <div className="w-full">
      ...
      {error ? (
        <p id={`${inputId}-error`} className="mt-1 font-assistant text-[12px] text-red-500">
          {error}
        </p>
      ) : null}

      {!error && hint ? (
        <p id={`${inputId}-hint`} className="mt-1 font-assistant text-[12px] text-[#AAAAAA]">
          {hint}
        </p>
      ) : null}
    </div>
