...
export default function SignInGoogleButton({ callbackUrl }: Props) {
  ...
  async function handleClick(e: React.MouseEvent<HTMLButtonElement>) {
    ...
    try {
      ...
      setError("אירעה שגיאה בהתחברות. בדקו הגדרות Google OAuth ונסו שוב.");
      setLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-2">
      <button
        type="button"
        className="flex w-full items-center justify-center gap-3 rounded-full border border-black/15 bg-white px-6 py-3.5 font-sans text-[15px] font-bold text-black shadow-[0_1px_4px_rgba(0,0,0,0.08)] transition-colors duration-200 hover:bg-black/5 disabled:pointer-events-none disabled:opacity-50"
        onClick={handleClick}
        disabled={loading}
      >
        <svg
      ...
      {error && (
        <p className="text-right font-assistant text-[12px] text-red-500" role="alert">
          {error}
        </p>
      )}
    </div>
